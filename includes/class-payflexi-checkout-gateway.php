...
class Payflexi_Checkout_Gateway extends WC_Payment_Gateway {
    /**
     * Class constructor, more about it in Step 3
    */
    public function __construct() {
        $this->id = 'payflexi-checkout';
        $this->method_title = 'PayFlexi Checkout';
		$this->method_description = sprintf( 'PayFlexi buy now pay later for WooCommerce allow your customers to pay in installment <a href="%1$s" target="_blank">Sign up</a> for a Payflexi account, and <a href="%2$s" target="_blank">get your API keys</a>.', 'https://payflexi.co', 'https://merchant.payflexi.co/settings' );
        ...
        add_action('woocommerce_api_payflexi_checkout_gateway', array( $this, 'verify_payflexi_transaction'));

        // Webhook listener/API hook
        add_action('woocommerce_api_payflexi_checkout_webhook', array( $this, 'process_webhooks' ) );
    }
    ...
    public function get_icon() {
		$icon  = '<img src="' . WC_HTTPS::force_https_url( plugins_url( 'assets/images/payflexi-wc.png' , PAYFLEXI_CHECKOUT_FILE ) ) . '" alt="PayFlexi Checkout" />';
    ...
    public function admin_notices() {
        ...
        if ( ! ( $this->public_key && $this->secret_key ) ) {
            echo '<div class="error"><p>' . sprintf( 'Please enter your Payflexi merchant details <a href="%s">here</a> to be able to use the Payflexi WooCommerce plugin.', admin_url( 'admin.php?page=wc-settings&tab=checkout&section=payflexi-checkout' ) ) . '</p></div>';
            return;
        }
	}
	/**
    ...
    */
    public function admin_options() {
        ...
        <h4>Optional: To avoid situations where bad network makes it impossible to verify transactions, set your webhook URL <a href="https://merchant.payflexi.co/settings" target="_blank" rel="noopener noreferrer">here</a> to the URL below<strong style="color: red"><pre><code><?php echo WC()->api_request_url( 'Payflexi_Checkout_Webhook' ); ?></code></pre></strong></h4>
    ...
    public function admin_enqueue_styles_scripts() {
		...
		$payflexi_checkout_admin_params = array(
			'plugin_url'    => PAYFLEXI_CHECKOUT_URL
		);

		wp_enqueue_script( 'payflexi_checkout_admin', PAYFLEXI_CHECKOUT_ASSETS_URL . '/js/pf-checkout-admin' . $suffix . '.js', array( 'jquery' ), PAYFLEXI_CHECKOUT_VERSION, true );	
		wp_localize_script( 'payflexi_checkout_admin', 'payflexi_checkout_admin_params', $payflexi_checkout_admin_params );
    }
    /**
    ...
    */
    ...
    public function receipt_page( $order_id ) {
        ...
        echo '<div id="payflexi_checkout_form"><form id="order_review" method="post" action="'. WC()->api_request_url( 'Payflexi_Checkout_Gateway' ) .'"></form><button class="button alt" id="payflexi-checkout-payment-button">Pay with PayFlexi</button> <a class="button cancel" href="' . esc_url( $order->get_cancel_order_url() ) . '">Cancel order &amp; restore cart</a></div>
        ';
    }
    /**
     * Verify Payflexi BNPL Request
     */
        ...
        if (isset($_REQUEST['payflexi_txnref'])){
            ...
            if ( ! is_wp_error( $request ) && 200 == wp_remote_retrieve_response_code( $request ) ) {
                ...
                if (!$payflexi_response->errors) {
                    ...
                    $order->add_order_note( sprintf( 'Payment via PayFlexi BNPL Checkout successful (Transaction Reference: %s)', $payflexi_ref ) );
